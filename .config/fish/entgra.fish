...
function ebdf
	...
	if test (count $argv) -gt 2
		...
		set dirs (etb $argv | sed "s#$work/##" | awk '{ print NR " " $1 }' | fzf -m --reverse | awk '{print $2}' | sed "s#^#$work/#")
	end
	for x in $dirs
		emif $x
		if test "$status" -gt 0
...
function ebdfa
	...
	if test (count $argv) -gt 1
		...
		set dirs (etba $argv | sed "s#$work/##" | awk '{ print NR " " $1 }' | fzf -m --reverse | awk '{print $2}' | sed "s#^#$work/#")
	end
	for x in $dirs
		emif $x
		if test "$status" -gt 0
